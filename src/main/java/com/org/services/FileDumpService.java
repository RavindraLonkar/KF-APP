...
public class FileDumpService {
	...
	@Autowired
	PGR_ReturnNewRepository pGR_ReturnNewRepository;
	@Autowired
	PIR_ProductionRepository pIR_ProductionRepository;
	@Autowired
	PIR_ReturnNewRepository pIR_ReturnNewRepository;
	@Autowired
	PIR_IssueRepository pIR_IssueRepository;
	@Autowired
	DiscardRepository discardRepository;
	@Autowired
	DispatchRepository dispatchRepository;
	@Autowired
	Dispatch_Return_MasterRepository dispatch_Return_MasterRepository;
	@Autowired
	DeliveryRepository deliveryRepository;
	@Autowired
	DeliveryToGreen_HouseNew1Repository deliveryToGreen_HouseNew1Repository;
	...
	public Response dumpFile(File file, String shift, String objectName) {
		...
		case "PIR1_SCANOPPCODEISSUE":
			...
			List<PIR1_SCanOpcodeIssue> pIR1_SCanOpcodeIssueNewList = (List<PIR1_SCanOpcodeIssue>) pIR1_SCanOpcodeIssueRepository
					.save((List<PIR1_SCanOpcodeIssue>) dataList);
			response = new Response(CommonConstants.KF_SCUCESS, pIR1_SCanOpcodeIssueNewList,
					CommonConstants.KF_SCUCESS_MESSAGE);
			break;
		case "PIR2_SCANOPPCODEISSUE":
			...
			dataList = customPIR2_SCanOpcodeIssueUtils.getMappedObjectList(fileContent, new PIR2_SCanOpcodeIssue(),
					CommonConstants.PIR2_SCANOPCODEISSUE);
			...
			List<PIR2_SCanOpcodeIssue> pIR2_SCanOpcodeIssueNewList = (List<PIR2_SCanOpcodeIssue>) pIR2_SCanOpcodeIssueRepository
					.save((List<PIR2_SCanOpcodeIssue>) dataList);
			response = new Response(CommonConstants.KF_SCUCESS, pIR2_SCanOpcodeIssueNewList,
					CommonConstants.KF_SCUCESS_MESSAGE);
		...
		case "PIR3_SCANOPPCODEISSUE":
			...
			dataList = customPIR3_SCanOpcodeIssueUtils.getMappedObjectList(fileContent, new PIR3_SCanOpcodeIssue(),
					CommonConstants.PIR3_SCANOPCODEISSUE);
			...
			for (PIR3_SCanOpcodeIssue pIR3_SCanOpcodeIssue : (List<PIR3_SCanOpcodeIssue>) dataList) {
				...
				pIR3_SCanOpcodeIssue.setPi_prshift(Integer.parseInt(shift));
			}

			List<PIR3_SCanOpcodeIssue> pIR3_SCanOpcodeIssueNewList = (List<PIR3_SCanOpcodeIssue>) pIR3_SCanOpcodeIssueRepository
					.save((List<PIR3_SCanOpcodeIssue>) dataList);
			response = new Response(CommonConstants.KF_SCUCESS, pIR3_SCanOpcodeIssueNewList,
					CommonConstants.KF_SCUCESS_MESSAGE);
			break;

		case "PIR4_SCANOPPCODEISSUE":
			dataList = getModels(PIR4_SCanOpcodeIssue.class);
			CustomFileUtils<PIR4_SCanOpcodeIssue> customPIR4_SCanOpcodeIssueUtils = new CustomFileUtils();
			dataList = customPIR4_SCanOpcodeIssueUtils.getMappedObjectList(fileContent, new PIR4_SCanOpcodeIssue(),
					"plant_loc,pi_PIR_No,pi_Scnopcode,pi_Branchcode,pi_Variety,pi_Corb,pi_Opcode,pi_Stage,pi_Week,pi_Day,pi_Shift,pi_Inoculation,pi_Bottles,pi_Date");

			for (PIR4_SCanOpcodeIssue pIR4_SCanOpcodeIssue : (List<PIR4_SCanOpcodeIssue>) dataList) {
				pIR4_SCanOpcodeIssue.setPi_Time(customPIR4_SCanOpcodeIssueUtils.getPrTime());
				pIR4_SCanOpcodeIssue.setPi_Date(customPIR4_SCanOpcodeIssueUtils.getDate());
				pIR4_SCanOpcodeIssue.setPi_prweek(Integer.valueOf(customPIR4_SCanOpcodeIssueUtils.getPrWeek()));
				pIR4_SCanOpcodeIssue.setPi_prday(Integer.valueOf(customPIR4_SCanOpcodeIssueUtils.getPrDay()));
				pIR4_SCanOpcodeIssue.setPi_pryear(Integer.valueOf(customPIR4_SCanOpcodeIssueUtils.getPrYear()));

				pIR4_SCanOpcodeIssue.setPi_prshift(Integer.valueOf(Integer.parseInt(shift)));
			}

			List<PIR4_SCanOpcodeIssue> pIR4_SCanOpcodeIssueNewList = (List<PIR4_SCanOpcodeIssue>) pIR4_SCanOpcodeIssueRepository
					.save((List<PIR4_SCanOpcodeIssue>) dataList);
			response = new Response("1", pIR4_SCanOpcodeIssueNewList, "File Uploaded Successfully.");
			break;
		...
		case "PIR_PRODUCTION":
			...
			List<PIR_Production> pIR_ProductionList = (List<PIR_Production>) pIR_ProductionRepository
					.save((List<PIR_Production>) dataList);
		...
		case "PIR_RETURN":
			...
			List<PIR_ReturnNew> pIR_ReturnNewList = (List<PIR_ReturnNew>) pIR_ReturnNewRepository
					.save((List<PIR_ReturnNew>) dataList);
		...
		case "PGR_PRODUCTION":
			...
			List<PGR_Production> pGR_ProductionList = (List<PGR_Production>) pGR_ProductionRepository
					.save((List<PGR_Production>) dataList);
		...
		case "PGR_ISSUENEW":
			...
			List<PGR_IssueNew> pgrdata = (List<PGR_IssueNew>) pGR_IssueNewRepository
					.save((List<PGR_IssueNew>) dataList);
		...
		case "PGR_RETURNNEW":
			...
			List<PGR_ReturnNew> pGR_ReturnNewList = (List<PGR_ReturnNew>) pGR_ReturnNewRepository
					.save((List<PGR_ReturnNew>) dataList);
		...
		case "CONTAMINATION":
			...
			List<Contamination> contaminationList = (List<Contamination>) contaminationRepository
					.save((List<Contamination>) dataList);
		...
		case "DISCARD":
			...
			dataList = customDiscard.getMappedObjectList(fileContent, new Discard(), CommonConstants.DISCARD_HEADER);
		...
		case "DISPATCH_RETURN_MASTER":
			...
			List<Dispatch_Return_Master> dispatch_Return_MasterList = (List<Dispatch_Return_Master>) dispatch_Return_MasterRepository
					.save((List<Dispatch_Return_Master>) dataList);
		...
		case "DELIVERY":
			...
			dataList = customDelivery.getMappedObjectList(fileContent, new Delivery(), CommonConstants.DELIVERY_HEADER);
		...
		case "DELIVERYTOGREEN_HOUSENEW1":
			...
			List<DeliveryToGreen_HouseNew1> deliveryToGreen_HouseNew1List = (List<DeliveryToGreen_HouseNew1>) deliveryToGreen_HouseNew1Repository
					.save((List<DeliveryToGreen_HouseNew1>) dataList);
